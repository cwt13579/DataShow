...
public class IndexController extends BaseController {
	...
	public void getTop10() {
		...
		if (resultList != null && resultList.size() > 0) {
			for (StatisticsRecord item : resultList) {
				...
				//BigDecimal value = v1.multiply(k1).add(v2.multiply(k2)).multiply(percent).setScale(2, BigDecimal.ROUND_UP);
				// DecimalFormat df = new DecimalFormat("0.00");
				// String str = df.format(value);
				
				BigDecimal value = item.getBigDecimal("score").setScale(2, BigDecimal.ROUND_UP);
	...
	public void getAllOrg() {
		...
		if (resultList != null && resultList.size() > 0) {
			for (StatisticsRecord item : resultList) {
			    BigDecimal value = BigDecimal.valueOf(item.getDouble("score")).setScale(2, BigDecimal.ROUND_UP);
				item.put("score", value);
