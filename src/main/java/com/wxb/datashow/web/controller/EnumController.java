...
public class EnumController extends BaseController {
	...
	public void dataStatusInvoke() {
		...
		for (DataStatus status : DataStatus.values()) {
			if (DataStatus.ENABLED.equals(status)
					|| DataStatus.DISABLED.equals(status)) {
	...
	public void ruleInvoke() {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		List<ProductRule> productRuleList = ProductRule.dao
				.getProductRuleBy(null);
	...
	public void logSourceInfoInvoke() {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		List<LogSourceInfo> logSourceInfoList = LogSourceInfo.dao.getLogSourceInfoList();
		for (LogSourceInfo info : logSourceInfoList) {
			Map<String, String> map = new HashMap<String, String>();
			map.put("value", String.valueOf(info.getSrcId()));
			map.put("note", info.getSrcName());
			list.add(map);
	...
	public void logCollectMachineInvoke() {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		List<LogCollectMachine> logCollectMachineList = LogCollectMachine.dao.getLogSourceInfoList();
		for (LogCollectMachine machine : logCollectMachineList) {
			Map<String, String> map = new HashMap<String, String>();
			map.put("value", String.valueOf(machine.getMachineId()));
			map.put("note", machine.getMachineName());
			list.add(map);
