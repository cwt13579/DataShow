 ...
 <div class="container-fluid">
        <div class="row page-header">
			...
			<div class="col-xs-9" style="text-align: right; padding-right: 0px;">
				<button type="button" class="btn btn-default btn_loading" onclick="addFinance()"><span class="glyphicon glyphicon-plus"></span>新增</button>
			</div>
		</div>
		<div class="row">
				<div class="col-xs-12">
							...
							<form id="search_form" method="post" class="form-inline">
								<div class="form-group">
									<label>产品名称</label> <input type="text" class="form-control"
										id="financeName" name="qm.finance_name"></input>
								</div>
  ...
  <script id="listScript" type="text/html">
		...
		<tr>
            ...
            <td>{{item.back_way}}</td>
            <td>{{item.invest_min}}-{{item.invest_max}}</td>
            <td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">{{item.finance_intro}}</td>
            <td>{{item.relate_product}}</td>
            <td>
                <a href="#" onclick="editFinance({{item.id}})"><span class="glyphicon glyphicon-edit"></span> 修改</a>
				<a href="#" onclick="deleteFinance({{item.id}})"><span class="glyphicon glyphicon-trash"></span> 删除</a>
			</td>
		</tr>
		{{/each}}
	</script>
  ...
  <script type="text/javascript">
    ...
    function getData(index) {
	    $.ajax({
			url: "/finance/financeListInvoke?current=" + index,
			method:"post",
    ...
    function addFinance() {
    	window.location="/finance/financeAdd";
    }
    function editFinance(id) {
    	window.location = '/finance/financeEdit?id=' + id;
    }
    function deleteFinance(id) {
    	layer.confirm('删除确认', '', function() {
    	  $.ajax({
    		url:"/finance/financeDeleteInvoke?id=" + id,
    		method : "post",
            beforeSend : function() {
    		},
