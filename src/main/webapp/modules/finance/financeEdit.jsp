 ...
 <div class="container-fluid">
			...
			<div class="col-xs-12">
				<div class="panel panel-default">
					<div class="panel-body">
						<form id="finance_update_form" method="post" class="form-horizontal">
						    <input type="hidden" name="finance.id" value="${finance.id}">
						  <div class="form-group">
						    	<label class="control-label col-xs-2">产品编号 <font color="red">*</font></label>
						    	<div class="col-xs-4"><input type="text" id="finance_code" name="finance.finance_code" value="${finance.finance_code}" placeholder="如：10001" class="form-control"/></div>
						  	</div>
						  	<div class="form-group">
						  		<label class="control-label col-xs-2">产品名称 <font color="red">*</font></label>
						    	<div class="col-xs-4"><input type="text" id="finance_name" name="finance.finance_name" value="${finance.finance_name}" placeholder="如：天天有利" class="form-control"/></div>
						  	</div>
						  	<div class="form-group">
						  		<label class="control-label col-xs-2">产品简介 <font color="red">*</font></label>
						    	<div class="col-xs-4"><input type="text" id="finance_intro" name="finance.finance_intro" value="${finance.finance_intro}" placeholder="如：收单资金自动买入" class="form-control"/></div>
						  	</div>
						  	<div class="form-group">
						  		<label class="control-label col-xs-2">对接产品 <font color="red">*</font></label>
						    	<div class="col-xs-4"><input type="text" id="relate_product" name="finance.relate_product" value="${finance.relate_product}" placeholder="如：理财货币基金" class="form-control"/></div>
						  	</div>
						  	<div class="form-group">
  ...
  <script type="text/javascript">
   $(document).ready(function() {
	   var financeType = "${finance.finance_type}";
	   var termType = "${finance.term_type}";
	   $("#finance_type").ui_select({schema:"FinanceType",needDefault:false,selectedValue:financeType.split(",")});
	   $("#term_type").ui_select({schema:"TermType",needDefault:false,selectedValue:termType.split(",")});
		$("#logo").fileinput({
	    	  language: 'zh', //设置语言
		      ...
		      initialPreview : "<img src='${finance.logo_path}' class='file-preview-image' />",
		 ...
		 $("#banner").fileinput({
		      ...
		      dropZoneEnabled: false,//是否显示拖拽区域
		      allowedFileExtensions : ['jpg', 'png','gif'],//接收的文件后缀
		      initialPreview : "<img src='${finance.banner_path}' class='file-preview-image' />",
	    });
	...
	function form_save(){
		...
		$.ajax({
			url : "/finance/financeUpdateInvoke",
			method : "post",
			data :  $("#finance_update_form").serialize(),
			beforeSend : function() {
				$('.img_loading').show();
