...
<html lang="en">
 ...
 <div class="container-fluid">
        <div class="row page-header">
			<div class="col-xs-3">
				<h4><span class="glyphicon glyphicon-th"></span> ftp任务管理</h4>
			</div>
            <div class="col-xs-9" style="text-align: right; padding-right: 0px;">
				<button type="button" class="btn btn-default btn_loading" onclick="addFtpTask()"><span class="glyphicon glyphicon-plus"></span>新增</button>
			</div>
		</div>
		<div class="row">
				<div class="col-xs-12">
						...
						<div class="panel-body" style="background-color: #eeeeee;">
							<form id="search_form" method="post" class="form-inline">
								<div class="form-group">
									<label>ftp任务名称</label> <input type="text" class="form-control"
										id="ftp_name" name="qm.ftp_name"></input>
								</div>
  ...
  <script id="listScript" type="text/html">
		...
		<tr>
            ...
            <td>{{item.ftp_user}}</td>
            <td>
                <a href="#" onclick="editFtpTask({{item.id}})"><span class="glyphicon glyphicon-edit"></span> 修改</a>
				<a href="#" onclick="deleteFtpTask({{item.id}})"><span class="glyphicon glyphicon-trash"></span> 删除</a>
			</td>
		</tr>
		{{/each}}
	</script>
  ...
  <script type="text/javascript">
    ...
    function getData(index) {
	    $.ajax({
			url: "/log/ftpTask/ftpTaskListInvoke?current=" + index,
			method:"post",
    ...
    function addFtpTask() {
    	window.location="/log/ftpTask/ftpTaskAdd";
    }
    function editFtpTask(id) {
    	window.location = '/log/ftpTask/ftpTaskEdit?id=' + id;
    }
    function deleteFtpTask(id) {
    	layer.confirm('删除确认', '', function() {
    	  $.ajax({
    		url:"/log/ftpTask/ftpTaskDeleteInvoke?id=" + id,
    		method : "post",
            beforeSend : function() {
    		},
