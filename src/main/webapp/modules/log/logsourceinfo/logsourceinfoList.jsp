...
<html lang="en">
 ...
 <div class="container-fluid">
        <div class="row page-header">
			<div class="col-xs-3">
				<h4><span class="glyphicon glyphicon-th"></span> 日志源管理</h4>
			</div>
            <div class="col-xs-9" style="text-align: right; padding-right: 0px;">
				<button type="button" class="btn btn-default btn_loading" onclick="addLogSourceInfo()"><span class="glyphicon glyphicon-plus"></span>新增</button>
			</div>
		</div>
		<div class="row">
				<div class="col-xs-12">
							...
							<form id="search_form" method="post" class="form-inline">
								<div class="form-group">
									<label>名称</label> <input type="text" class="form-control"
										id="srcName" name="qm.src_name"></input>
								</div>
  ...
  <script id="listScript" type="text/html">
		...
		<tr>
            <td>{{item.src_name}}</td>
            <td>{{item.src_type_str}}</td>
            <td>{{item.src_ip}}</td>
            <td>{{item.src_port}}</td>
            <td>
                <a href="#" onclick="editLogSourceInfo('{{item.src_id}}')"><span class="glyphicon glyphicon-edit"></span> 修改</a>
				<a href="#" onclick="deleteLogSourceInfo('{{item.src_id}}')"><span class="glyphicon glyphicon-trash"></span> 删除</a>
			</td>
		</tr>
		{{/each}}
	</script>
  ...
  <script type="text/javascript">
    ...
    function getData(index) {
	    $.ajax({
			url: "/log/logsourceinfo/logSourceInfoListInvoke?current=" + index,
			method:"post",
    ...
    function addLogSourceInfo() {
    	window.location="/log/logsourceinfo/logSourceInfoAdd";
    }
    function editLogSourceInfo(id) {
    	window.location = '/log/logsourceinfo/logSourceInfoEdit?src_id=' + id;
    }
    function deleteLogsourceInfo(id) {
    	layer.confirm('删除确认', '', function() {
    	  $.ajax({
    		url:"/log/logsourceinfo/logSourceInfoDeleteInvoke?src_id=" + id,
    		method : "post",
            beforeSend : function() {
    		},
