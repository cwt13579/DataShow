...
<html lang="en">
 ...
 <div class="container-fluid">
        <div class="row page-header">
			<div class="col-xs-3">
				<h4><span class="glyphicon glyphicon-th"></span> 修改产品</h4>
			</div>
			...
			<div class="col-xs-12">
				<div class="panel panel-default">
					<div class="panel-body">
						<form id="product_update_form" method="post" class="form-horizontal">
						    <input type="hidden" name="product.id" value="${product.id}">
						  	<div class="form-group">
						    	<label class="control-label col-xs-2">产品编号 <font color="red">*</font></label>
						    	<div class="col-xs-4"><input type="text" id="product_code" name="product.product_code"  value="${product.product_code}" placeholder="如：10001" class="form-control"/></div>
						  	...
						  	<div class="form-group">
						    	...
						    	<div class="col-xs-4"><input type="text" id="product_name" name="product.product_name" value="${product.product_name}" placeholder="如：财富宝" class="form-control"/></div>
						  	...
						  	<div class="form-group">
						    	...
						    	<div class="col-xs-4"><input type="text" id="product_intro" name="product.product_intro" value="${product.product_intro}" placeholder="如：银联商务携手xx银行" class="form-control"/></div>
						  	...
						  	<div class="form-group">
						    	...
						    	<div class="col-xs-4"><input type="text" id="institution" name="product.institution"  value="${product.institution}" placeholder="如：xx银行" class="form-control"/></div>
						  	</div>
						  	<div class="form-group">
						  		<label class="control-label col-xs-2">产品额度 <font color="red">*</font></label>
						    	<div class="col-xs-2"><input type="text" id="loan_amount" name="product.product_limit" value="${product.product_limit}" placeholder="如：30000" class="form-control"/></div>
						  	...
						  	<div class="form-group">
						    	...
						    	<div class="col-xs-2"><input type="text" id="loan_term" name="product.product_period" value="${product.product_period}" placeholder="如：180" class="form-control"/></div>
						    	<div class="col-xs-4"><label id="validate_loan_term" class="control-label validate_label" style="color: red;"></label></div>
						  	</div>
						  	<div class="form-group">
						  		<label class="control-label col-xs-2">产品月利率 <font color="red">*</font></label>
						    	<div class="col-xs-2 " ><input type="text" id="product_rate" name="product.product_rate" value="${product.product_rate}" placeholder="如：1-10" class="form-control"/></div>
						  	</div>
  ...
  <script type="text/javascript">
   $(document).ready(function() {
		...
		var drawsarr=draws.split(",");  
		var applysarr=applys.split(","); 
		$("input[name='product.interest_way']:radio").each(function(){ 
			var value = $(this).val();
			if(interestsarr == value){
				 $(this).prop('checked', true);
			}  
		});  
		$("input[name='product.draw_way']:radio").each(function(){ 
			var value = $(this).val();
			if(draws == value){
				 $(this).prop('checked', true);
			}  
		});  
		$("input[name='product.apply_way']:radio").each(function(){ 
			var value = $(this).val();
			if(applys == value){
				 $(this).prop('checked', true);
			}
		...
		$("#logo").fileinput({
		      ...
		      initialPreview : "<img src='${product.logo_path}' class='file-preview-image' />",
	...
	function form_save(){
		...
		$.ajax({
			url : "/product/productUpdateInvoke",
			method : "post",
			data :  $("#product_update_form").serialize(),
			beforeSend : function() {
				$('.img_loading').show();
