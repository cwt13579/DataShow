...
$(function	()	{
    ...
    function myBrowser(){
        ...
        if (isIE) {
            ...
            if(fIEVersion <= 8.0){
            	/*alert('IE8以下');*/
            	/*var IEModalHtml = '<div id="IEModal" style="position: fixed;top: 0;left: 0;right: 0;bottom: 0;background: #000;z-index:10000;">' +
						'<div style="position: absolute;height: 30px;line-height: 30px;top: 50%;margin-top: -15px;text-align: center;color: #ffffff;width: 100%">抱歉，暂不支持此浏览器！</div>' +
					'</div>';
				$('body').append(IEModalHtml);*/


				var IEModal = document.createElement('div');
				IEModal.id = 'IEModal';
				IEModal.style.position = 'fixed';
				IEModal.style.top = 0;
				IEModal.style.left = 0;
				IEModal.style.right = 0;
				IEModal.style.bottom = 0;
				IEModal.style.background = '#000';
				IEModal.style.zIndex = 10000;
				IEModal.innerHTML = '<div style="position:absolute;height:30px;line-height:30px;top:50%;margin-top:-15px;text-align:center;color:#ffffff;width: 100%">抱歉，暂不支持此浏览器！</div>';
				document.body.appendChild(IEModal);
			}
        }
	}
    myBrowser();
	...
	//changePlace
	$("#topChangePlace").click(function () {
		var isShow =  $("#topChangePlaceBox").hasClass('active');
		if(isShow){
            $("#topChangePlaceBox").removeClass('active');
		}else {
            $("#topChangePlaceBox").addClass('active');
		}
    });
