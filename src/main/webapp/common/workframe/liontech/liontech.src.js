...
LT=new function() {
	...
	this.requireJquery = function(noJqueryUi) {
		...
		if(LT!=top.LT&&top.SF){//模块页面
			var theme=top.SF.desktop.theme;
			if(!theme){
				theme="default";
			}
	...
	this.requireJqGrid=function(){
		...
		loadJs(LT.getBasePath()+"/common/workframe/jqgrid/jquery.jqGrid.min.js");
		loadCss(LT.getBasePath()+"/common/workframe/jqgrid/ui.jqgrid.css");
		return instance;
	};
	...
	this.infomation=function(message,param) {
		...
		if((ltInfomation=($(setting.parent).find(">#lt-infomation"))).length==0){
			ltInfomation=$("<div id='lt-infomation'></div>").css("position","absolute").appendTo($(setting.parent));
...
//					percent=percent+plus;
	...
	this.createWindow=function(param,closed){
		var p={parent:document.body,headHeight:22,draggable:true,block:false,resizable:false,pos:null,width:500,height:250,left:0,top:0,closable:true,maxable:false,minable:true,max:false};
		$.extend(p,param);
		if(p.pos=="center"){
		...
		var topArea=$("<div class='lt_windowTop ui-widget-header' style='cursor:pointer;position:absolute;width:100%;height:"+p.headHeight+"px;border-radius:5px 5px 0px 0px;border-width:1px!important;'></div>").appendTo(root);
		topArea.css("bolder-radius",5);
		var title=$("<div class='lt_theme_word_sm' style='float:left;margin-left:10px;height:22px;'></div>").appendTo(topArea);
		var oper=$("<div style='position:absolute;right:3px;height:22px;width:auto'></div>").appendTo(topArea);
		...
		if(p.resizable){
//			var operedDiv=$("<div></div>").css({position:"absolute",top:0,left:0,width:"100%",height:"100%",opacity:0.001,zIndex:9999,display:"none"});
			root.resizable({
				start:function(){
					LT.showDragFixDiv();
...
//					operedDiv.hide();
	...
	this.openForm=function(windowParam,formParam,callback){
		...
		var content=$("<div style='position:relative;width:100%;height:100%'></div>");
		var formArea=$("<div style='position:absolute;left:1px;right:1px;top:1px;bottom:43px;overflow:auto'></div>")
		...
		var okBt=this.createButton(100, 35, null, p.ok).appendTo(buttonArea);
		okBt.css({float:"right"});
		form.addButton=function(btName){
			var newBt= LT.createButton(100, 35, null, btName).css({float:"right"}).css("margin-right","8px").appendTo(buttonArea);
			if(!operAreaFlag){
	...
	this.createButton=function(w,h,style,text){
		...
		return $("<input type='button'></input>").val(text)
		.css({width:w,height:h,position:"relative",fontSize:h/4+4<14?14:h/4+4}).puibutton();
//		var root=$("<div></div>").width(w).height(h).css({overflow:"visible",position:"relative",cursor:"pointer"})
//		.css("text-align","center").css("line-height",h+"px").css("font-size",h-8+"px");
//		var img=$("<img></img>").attr("src",getResource("common/workframe/liontech/img/buttonBg/"+this.theme+".png"))
//		.css("opacity",0.8).appendTo(root).css({position:"absolute",left:0,top:0,width:"100%",height:"100%"});
//		var name=$("<div></div>").html(text).appendTo(root).css({position:"absolute",left:0,top:0,width:"100%",height:"100%",overflow:"hidden"});
...
//						tree.hideNode(node);
		...
		function specialFont(treeId, treeNode) {
			...
			return {};
//			return 	treeNode.fitFilter ? {"font-weight":"bold","background-color":"blue"} : {"font-weight":"normal","background-color":"transparent"};
		}
		return input;
	}
...
//									callBack(new AsynException("001", "未知的状态码"));
	...
	LT.requireJquery().requirePrimeUi(["growl","button"]).requireContextMenu();
