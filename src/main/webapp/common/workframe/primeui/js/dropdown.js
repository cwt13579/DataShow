...
$(function() {
    ...
    $.widget("primeui.puidropdown", {
        ...
        options: {
            ...
            propertyName:null,
            scrollHeight: 300,
            fromExist:true     //是否从现有的dom元素上进行创建（即该元素是否已添加到页面中）
        },
        ...
        refresh: function(items){ //chender
            ...
            if(this.options.data) {
                ...
                for(var i = 0; i < this.options.data.length; i++) {//chender
                    var choice = this.options.data[i];
                    if(choice.paramKey!=null)
                        this.element.append('<option value="' + choice.paramKey + '">' + choice.paramValue + '</option>');
                    else
                        this.element.append('<option value="' + choice.paramValue + '">' + choice.paramValue + '</option>');
                }
            }
...
//            });
            $this._bindItemEvents(this.items.filter(':not(.ui-state-disabled)'));//一次性绑定速度快些
            if(this.options.data){
            	this.label.html($(this.choices[0]).html());
            }else{
            	this.label.html("");
            }
            this._triggerChange();
        ...
        _dealHeader: function(){
    		var temp=new Array();
    		temp.push({paramKey:"",paramValue:this.options.header});
    		for(var i=0;i<this.options.data.length;i++){
    			temp.push(this.options.data[i]);
    		}
        ...
        _create: function() {
        	...
        	this.element.change(function(){
        		me.selectValue(me.element.val(),true);
        	});
            if(this.options.data) {
            	if(this.options.header){
            		this._dealHeader();
            ...
            if(this.options.fromExist){
            	...
            	this.container=$('<div class="pui-dropdown ui-widget ui-state-default ui-corner-all ui-helper-clearfix" />');
            	var html='<div class="ui-helper-hidden"></div>'
            		+'<div class="ui-helper-hidden-accessible"><input type="text" /></div>'
            		+(this.options.editable?'<input type="text" class="pui-dropdown-label pui-inputtext ui-corner-all">':'<label class="pui-dropdown-label pui-inputtext ui-corner-all"/>')
            		+'<div class="pui-dropdown-trigger ui-state-default ui-corner-right"><span class="ui-icon ui-icon-triangle-1-s"></span></div>';
            	this.container.html(html);
            	this.element.appendTo(this.container.find(">.ui-helper-hidden"));
            	this.focusElementContainer = this.container.find(">.ui-helper-hidden-accessible");
            	this.focusElement = this.focusElementContainer.children('input');
            	this.label = this.container.find(">.pui-dropdown-label");
            	this.menuIcon = this.container.find(">.pui-dropdown-trigger");           	
            }
            if(LT.simpleStyle){
            	this.container.addClass("lt_simpleStyle_item");
	        }
            this.panel = $('<div class="pui-dropdown-panel ui-widget-content ui-corner-all ui-helper-hidden pui-shadow" >'
            		...
            		+'<ul class="pui-dropdown-items pui-dropdown-list ui-widget-content ui-widget ui-corner-all ui-helper-reset"></ul>'
            		+'</div>'
            		+'</div>').appendTo(document.body);
            this.panel.css("overflow-y","auto");
            this.itemsWrapper = this.panel.find(">.pui-dropdown-items-wrapper");
            this.itemsContainer = this.itemsWrapper.find(">.pui-dropdown-items");
        ...
        _generateItems: function() {
            ...
            for(var i = 0; i < this.choices.length; i++) {
                ...
                willAppend+='<li data-label="' + optionLabel + '" class="pui-dropdown-item pui-dropdown-list-item ui-corner-all">' + content + '</li>';
            } 
            this.itemsContainer.html(willAppend);   
        ...
        _initDimensions: function() {
            var size={width:this.element[0].style["width"],height:this.element[0].style["height"]};
            size.width=size.width?size.width:"150px";
            size.height=size.height?size.height:"25px";
            this.container.css(size).css("padding-right","6px");


            this.label.css("line-height", size.height);
            this.label.css({height:"100%"});
        ...
        _selectItem: function(item, silent,inner) {
            ...
            if(shouldChange||inner) {//inner为true时代表是改变原生的下拉框地热值引起的事件，也需要改变显示的值
        ...
        _alignPanel: function() {        
            this.panel.css({left:'', top:'',width:this.container.width()+5});
            this.itemsWrapper.css("max-height",this.options.scrollHeight);
            var top=this.container.offset().top;
            var panelHeight=this.panel.height();
            var wh=$(document.body).height();
            var containerHeight=this.container.height();
            if(top+containerHeight+10+panelHeight<wh){//显示在下面
        	 this.panel.position({
                 my: 'left top',
                 at: 'left bottom',
                 of: this.container
                 });
            }else if(top-panelHeight-10>0){//显示在上面
            	 this.panel.position({
                     my: 'left bottom',
                     at: 'left top',
                     of: this.container
                     });
            }else{//显示在多的一边,并改变scroll的值
            	var bottomHeight=wh-top-containerHeight;
            	if(bottomHeight>top){//显示在下面
            		this.itemsWrapper.css("max-height",bottomHeight-10);
            		 this.panel.position({
                         my: 'left top',
                         at: 'left bottom',
                         of: this.container
            		...
            		this.itemsWrapper.css("max-height",top-10);
            		 this.panel.position({
                         my: 'left bottom',
                         at: 'left top',
                         of: this.container
...
//        	}
        ...
        _revert: function() {
            if(this.options.editable && this.customInput) {
            	...
            	if(this.preShowValue.index() < 0)
            		{
            		 this._highlightItem(this.items.eq(0));
            		}else
            			{
        ...
        selectValue : function(value,inner) {
            var option = this.choices.filter('[value="' + value + '"]');
            if(option.length==0){//非法的值，选中第一个
            	  this._selectItem(this.items.eq(0), true,inner);
            }else{
            	this._selectItem(this.items.eq(option.index()), true,inner);
            }
        },
